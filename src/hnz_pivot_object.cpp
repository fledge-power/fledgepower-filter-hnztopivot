...
Datapoint *HnzPivotObject::getCdc(Datapoint *dp)
{
    ...
    {
        throw HnzPivotObjectException("CDC type (" + HnzPivotCdcStr(m_pivotCdc) + ") does not match pivot class (" + HnzPivotClassStr(m_pivotClass) + ")");
    }
...
void HnzPivotObject::handleDetailQuality(Datapoint *detailQuality)
{
    ...
    for (Datapoint *child : *datapoints)
    {
        if (readBool(child, "badReference", m_badReference))
            continue;
        if (readBool(child, "failure", m_failure))
            continue;
        if (readBool(child, "inconsistent", m_inconsistent))
            continue;
        if (readBool(child, "inacurate", m_inacurate))
            continue;
        if (readBool(child, "oldData", m_oldData))
            continue;
        if (readBool(child, "oscillatory", m_oscillatory))
            continue;
        if (readBool(child, "outOfRange", m_outOfRange))
            continue;
        if (readBool(child, "overflow", m_overflow))
            continue;
    }
}
...
void HnzPivotObject::handleCdc(Datapoint *cdc)
{
    ...
    {
        Datapoint *sps = getChild(cdc, "ctlVal");
        if (sps == nullptr)
        {
            throw HnzPivotObjectException("stVal for type SPSTyp");
        }
    ...
    else if (m_pivotCdc == HnzPivotCdc::MV)
    {
        ...
        if (magI == nullptr)
        {
            throw HnzPivotObjectException("No mag i for type MvTyp");
        }
...
std::vector<Datapoint *> HnzPivotObject::toHnzTMObject(std::shared_ptr<HNZPivotDataPoint> exchangeConfig) const
{
    ...
    Datapoint *value = createDpWithValue("do_value", static_cast<long>(intVal));
...
void HnzPivotObject::toHnzValidityObject(std::vector<Datapoint *> &dps) const
{
    ...
    {
        Datapoint *valid = createDpWithValue("do_valid", static_cast<long>(1));
        dps.push_back(valid);
        Datapoint *outdated = createDpWithValue("do_outdated", static_cast<long>(1));
        dps.push_back(outdated);
    }
    else if (m_validity == HnzValidity::QUESTIONABLE)
    {
        Datapoint *valid = createDpWithValue("do_valid", static_cast<long>(0));
        dps.push_back(valid);
        Datapoint *outdated = createDpWithValue("do_outdated", static_cast<long>(1));
        dps.push_back(outdated);
    }
    else
    {
        Datapoint *valid = createDpWithValue("do_valid", static_cast<long>(0));
        dps.push_back(valid);
        Datapoint *outdated = createDpWithValue("do_outdated", static_cast<long>(0));
        dps.push_back(outdated);
    }
...
std::vector<Datapoint *> HnzPivotObject::toHnzTSObject(std::shared_ptr<HNZPivotDataPoint> exchangeConfig) const
{
    ...
    if (m_cause == 3)
    {
        ...
        Datapoint *val = createDpWithValue("do_value", static_cast<long>(intVal));
        hnzObject.push_back(val);

        Datapoint *ts = createDpWithValue("do_ts", static_cast<long>(
